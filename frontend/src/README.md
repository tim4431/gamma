...
| `editor/` | Outliner (`BlockTree.jsx`), CodeMirror (`BlockCmEditor.jsx`), undo history, Markdown and LaTeX editing, slash commands, the block-scope shortcut catalog (`blockCommands.js`) with the formatting runners (`markCommands.js`) and the pure fence scanner (`fences.js`) |
...
| `library/` | Library cards and browsing controls (`FileBrowser.jsx`), the Ctrl+P page palette and Ctrl+Shift+P command palette (`QuickOpen.jsx`), folder/page rules, title scoring, and `library.css` |
