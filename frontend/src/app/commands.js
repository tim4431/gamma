...
import { APP_COMMANDS, GROUP_NAVIGATION, GROUP_PAGE, GROUP_VIEW } from "./appCommands.js";
...
// The pane's order.
export const GROUPS = Object.freeze([GROUP_NAVIGATION, GROUP_PAGE, GROUP_VIEW, GROUP_NOTES, GROUP_FORMAT]);
...
// is [chords, what it does]; `enterNewNote` (Settings → Reading) swaps the
// two Enter rows, so the caller picks with `fixedKeys(enterNewNote)`.
export function fixedKeys(enterNewNote) {
  return [
    [[enterNewNote ? "Enter" : "Shift-Enter"], t("New note")],
    [[enterNewNote ? "Shift-Enter" : "Enter"], t("Line break (continues a list)")],
    [["Tab", "Shift-Tab"], t("Indent / outdent · hop between {} arguments in math · hop table cells")],
    [["Backspace"], t("On an empty note: delete it")],
    [["ArrowLeft", "ArrowRight"], t("At the text's edge: collapse / expand the children")],
    [["/"], t("Command menu in a note")],
    [["Escape"], t("Close popups, clear selections, cancel modes")],
  ];
}
