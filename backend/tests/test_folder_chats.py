...
def test_folder_rename_moves_buckets_by_prefix(guest):
    ...
    r = guest.post("/api/folders/rename", json={"src": "a", "dst": "b"})
...
def test_folder_rename_overwrites_empty_destination_row(guest):
    ...
    guest.post("/api/folders/rename", json={"src": "p", "dst": "q"})
...
def test_folder_rename_never_touches_root_bucket(guest):
    ...
    assert guest.post("/api/folders/rename", json={"src": "", "dst": "x"}).status_code == 400
    guest.post("/api/folders/rename", json={"src": "home", "dst": ""})  # only "home:home" would match
