...
def refresh(user: str, pages_conn: sqlite3.Connection, page_ids=None,
            batch: int = REFRESH_BATCH) -> int:
    """Bring the index up to date for the given pages (None = every page):
    rebuild the stale ones, at most ``batch`` per call. Returns how many are
    still stale afterwards (the "indexing" count a search reports)."""
    ...
    with sqlite3.connect(user_db_path(user, "data.db")) as data_conn:
        ...
        for page_id in stale[:batch]:
            index_page(pages_conn, data_conn, page_id, live[page_id])
        data_conn.commit()
    return max(0, len(stale) - batch)
...
def search_blocks(data_conn: sqlite3.Connection, match: str, limit: int,
                  page_ids=None) -> list[tuple[str, str, str]]:
    """bm25-ranked ``(block_id, page_id, snippet)`` hits for an FTS MATCH,
    restricted to page_ids when given (deleted pages linger until pruned, a
    scope may be a folder). A malformed MATCH is no results, not an error."""
    ...
    try:
        cur = data_conn.execute(
            ...
            (match, limit * 3 if allowed is not None else limit))
