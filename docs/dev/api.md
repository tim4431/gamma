...
| GET | `/server-config` | public: what the login page offers besides a password — `{cloud: {enabled, issuer}, password_login, registration, guest, guest_ttl_hours, demo, page_host}` (`cloud.enabled` false while the server still has to be connected; `guest` false on a share host; `guest_ttl_hours` how long a guest account lives; `demo` whether demo mode is on — [guests.md](guests.md); `page_host` the per-account page hostname pattern from `GAMMA_PAGE_HOST`, e.g. `{username}-pages.gammapdf.com`, "" when unset — how the app knows it was opened on a page host) ([cloud_accounts.md](cloud_accounts.md)) |
