...
- **Appearance**: the eight theme cards (`PictureChoices`), the interface
  ...
  switch with its live PDF sample, the sync pill's scope (a `Segmented`:
  synced pages only, or every page of a workspace that syncs some —
  `syncPillScope`, [mirror.md](mirror.md) "Publishing"), **Suggest tours**
